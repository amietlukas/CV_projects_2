...
def main():
  ...
  # TODO
  for R_candidate, t_candidate in possible_relative_poses:
    e_im1.SetPose(np.eye(3), np.zeros(3)) # First image/ camera as origin
    e_im2.SetPose(R_candidate, t_candidate) # Second image/ camera relative to the first one (iterate over solutions)
    candidate_points3D, _, _ = TriangulatePoints(K, e_im1, e_im2, e_matches)

    if candidate_points3D.size == 0:
        continue

    # if candidate_points3D.shape[0] > max_points:
    #   max_points = candidate_points3D.shape[0]
    #   best_pose = [R_candidate, t_candidate]

    # explicit count (z>0 in both cams)
    z1 = (np.eye(3) @ candidate_points3D.T + np.zeros((3,1)))[2] # cam1: R=I,t=0
    z2 = (R_candidate @ candidate_points3D.T + t_candidate[:, None])[2] # cam2: R,t
  ...
  # Final poses
  e_im1.SetPose(np.eye(3), np.zeros(3)) # First image/ camera as origin
  e_im2.SetPose(best_pose[0], best_pose[1]) # Second image/ camera relative to the first one (best)

  # quick sanity
  R2, t2 = e_im2.Pose()
  print("Initial baseline length:", np.linalg.norm(t2))
